...
game_path = 'C:/Users/CAD/Desktop/SnakeWithPython/'
...
def snakeDraw(snake, boxW, go):
    if go == 'R':
        head = pygame.transform.rotate(i_head, 270)
    if go == 'L':
        head = pygame.transform.rotate(i_head, 90)
    if go == 'U':
        head = i_head
    if go == 'D':
        head = pygame.transform.rotate(i_head, 180)

    gameDisplay.blit(head, (boxW * snake[-1][0], boxW * snake[-1][1]))

    for xy in snake[:-1]:
        specs = [boxW * xy[0], boxW * xy[1], boxW, boxW]
        pygame.draw.rect(gameDisplay, black, specs)
